function toTheGraveDamage( keys )
...
function toTheGraveDamageCritical( keys )
...
function toTheGraveSpreadTick	( event )

	-- Variables
	local caster = event.caster
	local target = event.target
	local ability = event.ability
	local dotDuration = ability:GetLevelSpecialValueFor("debuff_duration", 0)

	if target:HasModifier("modifier_necrolyte_attack_dot_debuff_datadriven") then
	else
	    ability:ApplyDataDrivenModifier(caster, target, "modifier_necrolyte_attack_dot_debuff_datadriven", {duration=dotDuration})
	end
