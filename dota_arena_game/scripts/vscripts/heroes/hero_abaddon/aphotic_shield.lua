...
function AphoticShield( event )
	...
	if IsValidEntity(target) then
		...
		local shield_size = 100 -- could be adjusted to model scale

		-- Strong Dispel
		--local RemovePositiveBuffs = false
		--local RemoveDebuffs = true
		--local BuffsCreatedThisFrameOnly = false
	--	local RemoveStuns = true--local RemoveExceptions = false
	--	target:Purge( RemovePositiveBuffs, RemoveDebuffs, BuffsCreatedThisFrameOnly, RemoveStuns, RemoveExceptions)
		...
		Timers:CreateTimer(0.01, function() 
			target.ShieldParticle = ParticleManager:CreateParticle("particles/units/heroes/hero_winter_wyvern/wyvern_arctic_burn_buff_ground_dust.vpcf", PATTACH_ABSORIGIN_FOLLOW, target)
	...
	-- Track how much damage was already absorbed by the shield
		...
		if not unit:HasModifier("modifier_borrowed_time") then
			...
			if unit.AphoticShieldRemaining <= 0 then
				...
				unit:RemoveModifierByName("modifier_external_ice_shield_absorb_crystal_maiden_buff_magical_dispellable_datadriven")
